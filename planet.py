...
def planetGravity(body, gravity, damping, dt):
    ...
    sq_dist = body.position.get_dist_sqrd(sunPosition)
    g = (
        (body.position - pymunk.Vec2d(sunPosition[0],sunPosition[1]))
        * -gravityStrength
...
def add_box(space,xcoord,ycoord):
    body = pymunk.Body()
    body.position = pymunk.Vec2d(xcoord, ycoord)
    body.velocity_func = planetGravity

    # Set the box's velocity to put it into a circular orbit from its
    # starting position.
    r = body.position.get_distance(sunPosition)
    v = math.sqrt(gravityStrength / r) / r
    body.velocity = (body.position - pymunk.Vec2d(sunPosition[0],sunPosition[1])).perpendicular() * v
    # Set the box's angular velocity to match its orbital period and
    # align its initial angle with its position.
    body.angular_velocity = v
    body.angle = math.atan2(body.position.y, body.position.x)

    box = pymunk.Poly.create_box(body, size=(1, 1))
    box.mass = 1
    box.friction = 0.7
    box.elasticity = 0
...
for x in range(3):
    add_box(space,300,random.randint(50,350))
...
while True:
    ...
    pygame.draw.circle(screen, pygame.Color("yellow"), sunPosition, 10)
